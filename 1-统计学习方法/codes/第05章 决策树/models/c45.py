...
实现 C4.5 决策树算法，用于分类任务。

说明：
- C4.5 算法基于信息增益率进行特征选择
- 信息增益率 = 信息增益 / 特征的经验熵
- 解决了 ID3 算法偏向选择取值较多特征的问题
...
class C45DecisionTree:
    def __init__(self, min_samples_split: int = 2, max_depth: int = float('inf'), 
        ...
        :param min_info_gain: 最小信息增益阈值，低于此值则不分割
        """
    ...
    def _feature_entropy(self, X: np.ndarray, feature_idx: int, verbose: bool = False) -> float:
        """
        计算特征的经验熵 IV(A) = -∑(|D_v|/|D| * log2(|D_v|/|D|))
        :param X: 特征矩阵
        ...
        if verbose:
            print(f"      ┌─ 计算特征熵 IV(A_{feature_idx}) = -∑(|D_v|/|D| * log2(|D_v|/|D|))")
            print(f"      │  特征 A_{feature_idx} 有 {len(feature_values)} 个取值: {sorted(feature_values)}")
            print(f"      │  总样本数 |D| = {total_samples}")
            print(f"      │")
        ...
        for idx, value in enumerate(feature_values):
            ...
            if p > 0:
                ...
                if verbose:
                    print(f"      │  ┌─ 取值 {value}:")
                    print(f"      │  │  样本数 |D_{value}| = {subset_size}/{total_samples} = {p:.4f}")
                    print(f"      │  │  项: ({subset_size}/{total_samples}) * log2({subset_size}/{total_samples}) = {term:.4f}")
                    if idx < len(feature_values) - 1:
                        print(f"      │  │")
                    terms.append(f"({subset_size}/{total_samples}) * log2({subset_size}/{total_samples})")
                    term_values.append(f"{term:.4f}")
        
        if verbose:
            print(f"      │")
            print(f"      │  ┌─ 汇总计算:")
            print(f"      │  │  IV(A_{feature_idx}) = -[" + " + ".join(terms) + "]")
            print(f"      │  │                  = -[" + " + ".join(term_values) + "]")
            print(f"      │  │                  = {feat_entropy:.4f}")
            print(f"      └─ 特征熵 IV(A_{feature_idx}) = {feat_entropy:.4f}")
    ...
    def _information_gain_ratio(self, X: np.ndarray, y: np.ndarray, feature_idx: int) -> float:
        """
        计算信息增益率 gR(D,A) = g(D,A) / IV(A)
        :param X: 特征矩阵
        :param y: 标签数组
        :param feature_idx: 特征索引
        :return: 信息增益率
        """
        info_gain = self._information_gain(X, y, feature_idx)
        feat_entropy = self._feature_entropy(X, feature_idx)
        
        # 如果特征熵为0，为了避免除零错误，返回0
        if feat_entropy == 0:
            return 0.0
        
        return info_gain / feat_entropy
    ...
    def _build_tree(self, X: np.ndarray, y: np.ndarray, depth: int = 0, used_features: set = None) -> dict:
        ...
        for i in available_features:
            ...
            # 计算信息增益
            info_gain = self._information_gain(X, y, i)
            # 计算特征熵（带详细打印）
            print(f"\n    计算特征熵 IV(A_{i}):")
            feat_entropy = self._feature_entropy(X, i, verbose=True)
            # 计算信息增益率
            gain_ratio = self._information_gain_ratio(X, y, i)
            feature_gains.append((i, info_gain, feat_entropy, gain_ratio))
            
            print(f"\n    - 信息增益 g(D,A_{i}) = H(D) - H(D|A_{i}) = {base_entropy:.4f} - {cond_entropy:.4f} = {info_gain:.4f}")
            if feat_entropy > 0:
                print(f"    - 信息增益率 gR(D,A_{i}) = g(D,A_{i}) / IV(A_{i}) = {info_gain:.4f} / {feat_entropy:.4f} = {gain_ratio:.4f}")
            else:
                print(f"    - 信息增益率 gR(D,A_{i}) = 0 (特征熵为0)")
            
            if gain_ratio > best_gain_ratio:
                best_gain_ratio = gain_ratio
                best_feature_idx = i
        
        # 如果没有找到有效分割特征或信息增益率过小，返回多数类
        if best_feature_idx == -1 or best_gain_ratio < self.min_info_gain:
            return {'type': 'leaf', 'class': self._majority_class(y)}
        
        print(f"\n最优特征: 特征 {best_feature_idx}, 信息增益率: {best_gain_ratio:.4f}")
    ...
    def print_tree(self, node: dict = None, indent: str = "", prefix: str = ""):
        ...
        if node['type'] == 'leaf':
            ...
            print(f"{indent}{prefix}特征 {node['feature_idx']} (信息增益率: {node.get('gain_ratio', 0):.4f})")
