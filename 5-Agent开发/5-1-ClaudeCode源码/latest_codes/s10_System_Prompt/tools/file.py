...
from system.paths import PROJECT_ROOT, resolve_in_project
...
def run_bash(command: str) -> str:
    if any(d in command for d in DENY_LIST):
        ...
        return "错误：危险命令已拦截"
    try:
        r = subprocess.run(command, shell=True, cwd=PROJECT_ROOT,
...
def run_read(path: str, limit: int = None) -> str:
    try:
        text = resolve_in_project(path).read_text(encoding="utf-8")
...
def run_glob(pattern: str) -> str:
    ...
    try:
        results = []
        for match in g.glob(pattern, root_dir=PROJECT_ROOT):
