...
if __name__ == "__main__":
    ...
    print("  3 - 宽松模式（放行所有工具调用操作）")
    choice = input("请输入 [1/2，默认 3 全部放行]: ").strip()
    mode_map = {"1": PermissionMode.AUTO_DENY, "2": PermissionMode.LEARNING, "3": PermissionMode.LENIENT}
    permission_manager.set_mode(mode_map.get(choice, PermissionMode.LENIENT))
    logger.info(f"当前权限模式: {permission_manager.mode.value}")

    queries = ["你好", "用todo tool 看看桌面目录下有哪些pdf文件 然后读取第3个pdf 为我写读后感"]
    state = State()
    i = 0
