...
def predict_labels(model, X_test_vec, num_class):
...
def train_once(train_data, X_test_vec, y_test, num_class, n_estimators, max_depth):
    params = build_params(num_class, max_depth)
    model = lgb.train(params, train_data, num_boost_round=n_estimators)
    y_pred = predict_labels(model, X_test_vec, num_class)
    acc = accuracy_score(y_test, y_pred)
    return acc


def plot_lightgbm_acc(ds_name, acc_matrix):
    FIGURES_DIR.mkdir(parents=True, exist_ok=True)
    fig_path = FIGURES_DIR / f"lightgbm_depth_tuning_{ds_name}.png"
    ...
    title = f"LightGBM Performance on {ds_name}\n(Learning Rate: {FIXED_LEARNING_RATE})"
...
def run_grid_search(ds_name, X_train, X_test, y_train, y_test, target_names=None):
    ...
    final_model = lgb.train(build_params(num_class, best_depth), train_data, num_boost_round=best_n_est)
    y_pred_best = predict_labels(final_model, X_test_vec, num_class)
    if target_names:
        print(classification_report(y_test, y_pred_best, target_names=target_names))

    plot_lightgbm_acc(ds_name, acc_matrix)
